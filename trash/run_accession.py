...
existing_data = {}
with open(data_dir+'collection.json') as json_file:
...
def classify(image_file):
    # print("classifying",image_file)
    path = abspath(image_file)
    return all_data_from_image(path)
...
def makeObject(fname_base, use_top_cam):
    ...
    if use_top_cam:
        ...
        results = all_data_from_image(image_dir+fname_base+"_top_0.jpg") 
    else:
        print(Fore.CYAN+"USING FRONT CAMERA IMAGE FOR CLASSIFICATION")
        results = all_data_from_image(image_dir+fname_base+"_front_2.jpg")
    #print(results)
    collection = []   
    ...
    new_object["colours"]=getColours( image_dir+fname_base+"_top_0.jpg" )
...
def getColours(fpath):
    get_hex_palette(fpath)
 
def getWatsonColours(watson_results):
    colours = []
    try:
